...
// 
// state and tx_state are native to dualopen
// move them to a new header interactive/something.h
// might be able make a new struct with less stuff
// inprog_tx_state
// 
// branch is https://github.com/niftynei/lightning/tree/nifty/splice

// bool run_tx_interactive(struct state *state,
// 			       struct tx_state *tx_state,
...
static void add_funding_output(struct inprog_tx_state *tx_state,
...
static void opener_start(struct state *state, u8 *msg)
{
	 ...
	 */
	add_funding_output((struct inprog_tx_state*)tx_state, (struct inprog_state*)state, total);
