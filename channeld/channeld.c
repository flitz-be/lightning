...
struct peer {
	...
	/* The amount the user told us to commit to for a splice */
			    ...
			    struct tlv_commitment_signed_tlvs *cs_tlv)
{
	...
	msg_for_master
		= towire_channeld_got_commitsig(NULL,
					       ...
					       changed,
					       committx,
					       NULL); /* DTODO: Save the splice commitment info */
...
static void splice_accepter(struct peer *peer, const u8 *inmsg)
{
	...
	accepter_amount = amount_msat_to_sat_round(peer->channel->view->owed[LOCAL]);
...
static void splice_intiator(struct peer *peer, const u8 *inmsg)
{
	...
	input_index = ictx->desired_psbt->tx->num_inputs;
...
static void splice_intiator_user_signed(struct peer *peer, const u8 *inmsg)
{
	...
	chan_output_index = find_channel_output(peer, signed_psbt);
